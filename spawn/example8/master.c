...
//  printf("intra rank = %d \n\n\n",intra_rank);
 ...
 if ( rank == 0 ) {
  ...
  int exit_type = 0;
  MPI_Status status, arraystatus;
  int flag = 0, count, arrayflag=0;
  int end = 0;
...
//		MPI_Isend(&exit_type,1,MPI_INT, 1,0,intra_comm2,&request);
  ...
  MPI_Iprobe(MPI_ANY_SOURCE,MPI_ANY_TAG,intra_comm2,&arrayflag, &arraystatus);  
  int arraysize;
  PetscScalar *array;
  MPI_Get_count(&arraystatus,MPI_INT,&arraysize);
  if(flag && arraysize !=1){
    MPI_Get_count(&arraystatus,MPIU_SCALAR,&arraysize);
    printf("msg >>> Master recv array size = %d ", arraysize);
    array = (PetscScalar *) malloc(arraysize*sizeof(PetscScalar));
    MPI_Recv(array,arraysize, MPIU_SCALAR, arraystatus.MPI_SOURCE,arraystatus.MPI_TAG,intra_comm2,&arraystatus);
    for(i=0;i<5;i++){
        printf("Master recv array[%d] = %f\n", i, array[i]);
    }
  }
