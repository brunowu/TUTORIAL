/*mpicc -o spawnMerge spawn_merge.c
 *mpirun -np 1 spawnMerge
*/
#include <stdio.h>
#include <stdlib.h>
#include <mpi.h>
int main(int argc, char **argv)
{
    ...
    if (!initialized) {
        ...
        if (boss_comm != MPI_COMM_NULL) {
            ...
            MPI_Comm_spawn("spawnMerge",args,2,MPI_INFO_NULL,0,comm_1,&intercomm_1,(int*)MPI_ERRCODES_IGNORE);

            printf("]> Master task: second spawn\n");
            MPI_Comm_dup(MPI_COMM_SELF,&comm_2);
            MPI_Comm_spawn("spawnMerge",args,2,MPI_INFO_NULL,0,comm_2,&intercomm_2,(int*)MPI_ERRCODES_IGNORE);
