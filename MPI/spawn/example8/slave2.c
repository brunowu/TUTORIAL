...
//  printf("\n\n HELLLLLLO \n\n");
  ...
  PetscScalar *sended_buffer;
  sended_buffer = (PetscScalar *)malloc(5*sizeof(PetscScalar));
  
  for(i=0;i<5;i++){
        sended_buffer[i] = (PetscScalar)array[i];
	printf("sended buffer[%d] = %f\n", i, sended_buffer[i]);
  }
 
  MPI_Isend(sended_buffer,5,MPIU_SCALAR,0,1,intra_comm2,&request); 
